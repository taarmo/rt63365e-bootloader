...
void W_AddFile(char* filename)
{
	...
	uart_printf("handle: %x \n\r",handle);
	uart_printf("sizeof(header): %x \n\r",sizeof(header));
	...
	uart_printf("Header.numlumps: %x y sizeof(filelump_t): %x \n\r",header.numlumps, sizeof(filelump_t));
	uart_printf("ESTO ES LA LENGHT %x \n\r", length);
        fileinfo = allocated = doom_malloc(length);
	uart_printf("ESTO ES FILEINFO: %x \n\r", fileinfo);
	uart_printf("Header.infotableofs: %x \n\r", header.infotableofs);
