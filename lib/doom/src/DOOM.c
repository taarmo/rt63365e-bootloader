...
#include "../../dlmalloc/sysinc/malloc.h"

extern u32 time_init;

int doom_strcmp(const char* , const char*);
...
static unsigned int file_pos;
static unsigned int size_file = sizeof(outputBuffer);
...
void* doom_open_impl(const char* filename, const char* mode)
{
	if (doom_strcmp(filename, "/home/tarmo/Escritorio/doom/doom1.wad") == 0) {
...
int doom_read_impl(void* handle, void *buf, int count)
{
	for (int i = 0; i < count; i++) {
		*((unsigned char *) buf + i) = *((unsigned char *) handle + file_pos);
		file_pos++;
	}
	return count;
}
