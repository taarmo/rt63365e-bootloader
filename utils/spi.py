...
def spi_write(to,data,size):
    ...
    p.write32(0xbfbc0004, (to&0x00ffffff)<<8 | data[0])
                                                                    #opcode
    reg = (reg & ~SPI_CTL_TX_RX_CNT_MASK) | n_bytes_data | SPI_ENABLE | (0x02 << 24)
...
#    p.write32(0xbfbc0008+i, data[4+i] | data[2+i] << 8 | data[1+i] << 16 | data[0+i]<<24) 
#
...
#
#                                               # byte 3
#    p.write32(0xbfbc0004, (to&0x00ffffff)<<8 | (data[3+i]))
#                                                                    #opcode
#    reg = (reg & ~SPI_CTL_TX_RX_CNT_MASK) | n_bytes_data | SPI_ENABLE | (0x02 << 24)
#    p.write32(0xbfbc0000, reg)
#    #p.write32(0xbfbc0038,0x1)
